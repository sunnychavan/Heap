...
/* NetId(s): Tcc86, Sc2448

 * Name(s): Tyler Carreja, Sunny Chavan
 * What I thought about this assignment: We though this assignment was a great way to gain experience using 
 * the heap data structure. It was also interesting to simultaneously make sure that each function we wrote
 * was within the desired complexity.
 *
 *
 */
...
public class Heap<T> {
    ...
    void bubbleDown(int h) {
        ...
        while (2 * h + 1 < size) {
        	int child1= compareTo(h, 2 * h + 1);       
            int child2_index = 2 * h + 2; 
            
            if(child2_index < size) {
            	int child2= compareTo(h, 2 * h + 2);
            	if (child1 == -1 && child2 == -1) {            		
            		int q = compareTo(2 * h + 1, 2 * h + 2);
            		if (q == -1) {
            			swap(h, 2 * h + 2);
            			h= 2 * h + 2;
            		}
